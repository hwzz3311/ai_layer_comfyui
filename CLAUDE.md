...
- **Edge color inpaint (v0.14.0)**: after the defringe pass, both pipelines call `_edge_color_inpaint(rgb, alpha_clean, ring_px=2, radius=3)` to rewrite the 1–2 px alpha-boundary ring with the nearest interior color via `cv2.inpaint(TELEA)`. Rationale: `VR_AlphaStepify` hard-thresholds soft alpha (~0.5–0.7 ramp pixels jump to 1.0), but their RGB is anti-aliased mix of subject + background. Without this pass the saved PNG carries a halo of background-tinted color along the silhouette that vectorizers trace as a thin wrong-color rim. The inpaint mask is only the eroded-vs-original ring, so cost is tiny relative to the bilateral / k-means already in the pipeline. Don't remove this step — it's part of the "clean for downstream vectorization" output contract.
